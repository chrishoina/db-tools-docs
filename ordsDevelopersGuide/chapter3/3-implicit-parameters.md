...
e.g. These options are enough to trigger the usage of body_json (and send the files to the ORDS package).
dbms_output.put_line(:body_json);
htp.p(:body_json);
...
BEGIN
    L_BODY_JSON := json_value(L_BODY_JSON,'$.submitted_by');
    HTP.P('Submitted By: ' || L_BODY_JSON );
